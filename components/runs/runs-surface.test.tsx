...
describe("RunsSurface", () => {
  ...
  describe("favicon for first displayed target", () => {
    it("shows favicon corresponding to first target url", () => {
      const row = buildRow({
        targetUrls: ["https://first-target.com", "https://second-target.com"],
        faviconUrl: "https://first-target.com/favicon.ico",
      })
      const { container } = render(<RunsSurface rows={[row]} sortOrder="newest" onToggleSortOrder={mockToggleSort} />)

      const images = container.querySelectorAll("img")
      expect(images[0]).toHaveAttribute("src", "https://first-target.com/favicon.ico")
    })

    it("renders the first target without its scheme", () => {
      render(
        <RunsSurface
          rows={[buildRow({ targetUrls: ["https://first-target.com/", "https://second-target.com/about"] })]}
          sortOrder="newest"
          onToggleSortOrder={mockToggleSort}
        />,
      )

      expect(screen.getAllByText("first-target.com").length).toBeGreaterThan(0)
    })
  })
