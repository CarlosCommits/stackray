...
function buildMockRunsRows() {
  return buildRunsRows(
    ...
    (scanId) => (scanId === "scn_01J_demo_recent" ? ["https://tpss.coop"] : ["https://queue.example.com"]),
    (_scanId, firstTarget) => (firstTarget === "https://tpss.coop" ? "https://tpss.coop/favicon.ico" : null),
  )
}
...
describe("/runs row contract", () => {
  ...
  it("builds a completed row with visible target urls and hidden count", () => {
    ...
    const completedRow: RunsRow = buildRunsRow(
      ...
      ["https://tpss.coop", "tpss.coop", "shop.tpss.coop", "checkout.tpss.coop"],
      "https://tpss.coop/favicon.ico",
    )
    ...
    expect(completedRow.targetUrls).toEqual(["https://tpss.coop", "tpss.coop", "shop.tpss.coop"])
    expect(completedRow.hiddenTargetCount).toBe(1)
    expect(completedRow.faviconUrl).toBe("https://tpss.coop/favicon.ico")
  ...
  it("builds page-facing rows for runs consumers", () => {
    ...
    expect(rows[0]?.faviconUrl).toBe("https://tpss.coop/favicon.ico")
